...
function parsePatternDraft(draftJson: string, originalText: string, name: string) {
  const fallback = parsePattern(originalText, name);
  if (!draftJson.trim()) return fallback;
  try {
    return sanitizeParsedPattern(JSON.parse(draftJson), name, detectLanguage(originalText));
  } catch (error) {
